...
function initEventListeners() {
    ...
    // Close modal button for password modal setup is moved to initializeApp
...
function hideChangePasswordModal() {
    ...
    try {
        // Hide the modal with animation support
        changePasswordModal.classList.remove('show');
        changePasswordModal.style.display = 'none';

        // Reset the form if it exists
        const form = changePasswordForm || changePasswordModal.querySelector('form');
        if (form) {
            form.reset();
        ...
        // Clear messages more comprehensively
        const messages = changePasswordModal.querySelectorAll('.message, .auth-message');
        messages.forEach(messageDiv => {
            messageDiv.textContent = '';
            messageDiv.className = messageDiv.className.replace(/\b(show|error|success)\b/g, '').trim();
            messageDiv.style.display = 'none';
        });

        // Reset any loading states
        const submitBtn = changePasswordModal.querySelector('button[type="submit"]');
        if (submitBtn) {
            submitBtn.disabled = false;
            submitBtn.innerHTML = submitBtn.getAttribute('data-original-text') || 'Change Password';
        }

        // Remove focus trap and restore previous focus
        if (document.activeElement && changePasswordModal.contains(document.activeElement)) {
            document.activeElement.blur();
        }

        console.log('Change password modal hidden successfully');

    } catch (error) {
        console.error('Error hiding change password modal:', error);
    }
}
